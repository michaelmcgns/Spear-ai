...
function computeSentiment(score: number): { label: string; color: string; pct: number } {
  if (score > 1)  return { label: 'Positive', color: '#2C4A75', pct: Math.min(100, 60 + score * 12) }
...
function cardStyle(ct: CardType) {
  if (ct === 'closing') return {
    bg: '#F2F6FB', border: 'rgba(44,74,117,0.3)', accent: '#2C4A75',
    badgeBg: 'rgba(44,74,117,0.12)', badgeColor: '#2C4A75', labelColor: '#2C4A75',
    innerBg: '#E8EEF5', responseLabelColor: '#2C4A75',
  }
  if (ct === 'buying_signal') return {
    bg: '#F2F6FB', border: 'rgba(44,74,117,0.28)', accent: '#2C4A75',
    badgeBg: 'rgba(44,74,117,0.12)', badgeColor: '#2C4A75', labelColor: '#2C4A75',
    innerBg: '#E8EEF5', responseLabelColor: '#2C4A75',
  }
  if (ct === 'disc') return {
...
export default function LiveCallPage() {
  ...
  const scoreColor = score >= 7.5 ? '#2C4A75' : score >= 5.5 ? '#C9A84C' : '#C0392B'
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', backgroundColor: '#F5F0E8', fontFamily: 'var(--font-space, system-ui, sans-serif)', overflow: 'hidden' }}>
      ...
      {/* ── Header ── */}
      <header style={{ backgroundColor: '#14213D', height: 56, flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 20px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <span style={{ color: '#2C4A75', opacity: 0.5 }}>|</span>
          <span style={{ color: '#C8D2E0', fontWeight: 700, fontSize: 14, letterSpacing: '0.06em' }}>SPEAR LIVE</span>
          <span style={{ padding: '3px 9px', borderRadius: 20, backgroundColor: 'rgba(44,74,117,0.2)', border: '1px solid rgba(44,74,117,0.4)', fontSize: 9, fontWeight: 800, color: '#2C4A75', letterSpacing: '0.09em' }}>
          ...
          {isLive && discProfile && (() => {
            const discHdr: Record<string, { bg: string; border: string; color: string }> = {
              ...
              S: { bg: 'rgba(44,74,117,0.15)',  border: 'rgba(44,74,117,0.35)',  color: '#2C4A75' },
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          {isLive && (
            <div style={{ display: 'flex', alignItems: 'baseline', gap: 4 }}>
              <span style={{ fontSize: 9, fontWeight: 700, letterSpacing: '0.1em', color: 'rgba(200,210,224,0.55)' }}>SCORE</span>
          ...
          {isLive && <span style={{ color: '#2C4A75', opacity: 0.4 }}>|</span>}
      ...
      {!isLive && status !== 'ended' && (
        <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
          <div style={{ backgroundColor: '#FDFAF5', border: '1px solid #DDD5BB', borderRadius: 16, padding: '36px 32px', maxWidth: 480, width: '100%', boxShadow: '0 4px 24px rgba(0,0,0,0.06)' }}>
            <div style={{ width: 48, height: 48, borderRadius: '50%', backgroundColor: 'rgba(44,74,117,0.12)', border: '1px solid rgba(44,74,117,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px' }}>
              <Mic size={20} style={{ color: '#2C4A75' }} />
            </div>
              ...
              {FOCUS_OPTIONS.map(opt => {
                ...
                return (
                  <button
                    ...
                    style={{ padding: '9px 12px', borderRadius: 8, border: `1px solid ${active ? '#2C4A75' : '#DDD5BB'}`, backgroundColor: active ? '#14213D' : '#F5F0E8', color: active ? '#C8D2E0' : '#5A5448', fontSize: 12, fontWeight: active ? 700 : 500, cursor: 'pointer', fontFamily: 'inherit', textAlign: 'left', transition: 'all 0.12s' }}
                  >
                    {opt.label}
                  </button>
                )
            ...
            <button
              onClick={startCall}
              style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 7, padding: '13px 0', borderRadius: 10, backgroundColor: '#14213D', color: '#C8D2E0', fontWeight: 700, fontSize: 14, border: 'none', cursor: 'pointer', fontFamily: 'inherit', width: '100%' }}
            >
          ...
          <div style={{ flex: 3, display: 'flex', flexDirection: 'column', borderRight: '1px solid #DDD5BB', overflow: 'hidden' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', padding: '0 12px', height: 34, backgroundColor: '#FDFAF5', borderBottom: '1px solid #DDD5BB', flexShrink: 0, gap: 2 }}>
              {PHASES.map((p, i) => (
                <div key={p} style={{ display: 'flex', alignItems: 'center', flex: 1 }}>
                  <div style={{ flex: 1, textAlign: 'center', padding: '4px 2px', borderRadius: 4, backgroundColor: phase === i ? '#14213D' : phase > i ? 'rgba(44,74,117,0.12)' : 'transparent', fontSize: 9, fontWeight: 700, letterSpacing: '0.04em', color: phase === i ? '#C8D2E0' : phase > i ? '#2C4A75' : '#B8AFA0', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                    {p}
                  </div>
            ...
            <div style={{ padding: '7px 12px', backgroundColor: '#FDFAF5', borderBottom: '1px solid #DDD5BB', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                ...
                {isLive && (
                  <span style={{ fontSize: 10, color: '#2C4A75', backgroundColor: 'rgba(44,74,117,0.1)', border: '1px solid rgba(44,74,117,0.3)', borderRadius: 5, padding: '2px 7px', fontWeight: 700, letterSpacing: '0.03em' }}>
                    ⌨️ Press <kbd style={{ fontFamily: 'monospace', background: '#14213D', color: '#C8D2E0', borderRadius: 3, padding: '0 5px', fontSize: 10 }}>SPACE</kbd> to switch Agent / Prospect
                  </span>
                )}
              </div>
              <div
              ...
              >
                ...
                <div style={{
                  ...
                  width: 'calc(50% - 4.5px)',
                  backgroundColor: '#14213D', borderRadius: 6,
                  transition: 'left 0.2s cubic-bezier(0.4,0,0.2,1)',
                  pointerEvents: 'none',
                }} />
                <span style={{ position: 'relative', padding: '7px 16px', fontSize: 12, fontWeight: 700, letterSpacing: '0.04em', color: speaker === 'agent' ? '#C8D2E0' : '#7A7060', transition: 'color 0.2s', userSelect: 'none', minWidth: 64, textAlign: 'center' }}>
                  Agent
                </span>
                <span style={{ position: 'relative', padding: '7px 16px', fontSize: 12, fontWeight: 700, letterSpacing: '0.04em', color: speaker === 'prospect' ? '#C8D2E0' : '#7A7060', transition: 'color 0.2s', userSelect: 'none', minWidth: 72, textAlign: 'center' }}>
                  Prospect
                </span>
              </div>
            </div>
            ...
            {lines.length > 0 && (
              <div style={{ padding: '5px 12px 7px', backgroundColor: '#FDFAF5', borderBottom: '1px solid #DDD5BB', flexShrink: 0 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                  <span style={{ fontSize: 10, fontWeight: 700, color: '#2C4A75' }}>YOU {agentPct}%</span>
                ...
                <div style={{ height: 5, borderRadius: 3, overflow: 'hidden', display: 'flex' }}>
                  <div style={{ width: `${agentPct}%`, backgroundColor: '#2C4A75', transition: 'width 0.5s ease' }} />
                ...
                <>
                  {lines.map(line => (
                    <div key={line.id} style={{ display: 'flex', gap: 8, alignItems: 'flex-start' }}>
                      ...
                      <span style={{ fontSize: 9, fontWeight: 800, letterSpacing: '0.07em', flexShrink: 0, marginTop: 4, minWidth: 52, color: line.speaker === 'agent' ? '#2C4A75' : '#8C6D2F' }}>
                  ...
                  {interim && (
                    <div style={{ display: 'flex', gap: 8, alignItems: 'flex-start' }}>
                      ...
                      <span style={{ fontSize: 9, fontWeight: 800, letterSpacing: '0.07em', flexShrink: 0, marginTop: 4, minWidth: 52, color: speaker === 'agent' ? '#2C4A75' : '#8C6D2F' }}>
              ...
              <div style={{ flexShrink: 0, borderTop: '1px solid #DDD5BB', backgroundColor: '#FDFAF5', padding: '8px 12px', display: 'flex', gap: 7, alignItems: 'center' }}>
                ...
                >
                  <div style={{
                    ...
                    width: 'calc(50% - 3px)',
                    backgroundColor: '#14213D', borderRadius: 4,
                    transition: 'left 0.2s cubic-bezier(0.4,0,0.2,1)',
                    pointerEvents: 'none',
                  }} />
                  <span style={{ position: 'relative', padding: '5px 10px', fontSize: 10, fontWeight: 700, letterSpacing: '0.04em', color: speaker === 'agent' ? '#C8D2E0' : '#7A7060', transition: 'color 0.2s', userSelect: 'none', minWidth: 40, textAlign: 'center' }}>
                    You
                  </span>
                  <span style={{ position: 'relative', padding: '5px 10px', fontSize: 10, fontWeight: 700, letterSpacing: '0.04em', color: speaker === 'prospect' ? '#C8D2E0' : '#7A7060', transition: 'color 0.2s', userSelect: 'none', minWidth: 56, textAlign: 'center' }}>
                    Prospect
                  </span>
                </div>
                <input
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '0 0 12px' }}>
                ...
                {cards.length > 0 && (
                  <div style={{ display: 'flex', gap: 5, alignItems: 'center' }}>
                    {cards.filter(c => c.cardType !== 'objection').length > 0 && (
                      <span style={{ fontSize: 10, fontWeight: 800, color: '#2C4A75', backgroundColor: 'rgba(44,74,117,0.1)', padding: '2px 8px', borderRadius: 10 }}>
                      ...
                      <div style={{ padding: '9px 12px' }}>
                        ...
                        {card.nextMove && (
                          <p style={{ margin: 0, fontSize: 10, fontWeight: 700, letterSpacing: '0.06em', color: cs.accent }}>
                            SIGNALS &nbsp;<span style={{ fontWeight: 400, color: '#5A6480', letterSpacing: 0 }}>{card.nextMove}</span>
                          </p>
                        )}
                      </div>
                    </div>
                  ...
                  return (
                    <div key={card.id} style={{ borderRadius: 10, border: `1px solid ${cs.border}`, overflow: 'hidden', backgroundColor: cs.bg }}>
                      ...
                      <div style={{ padding: '9px 12px' }}>
                        ...
                        <p style={{ margin: 0, fontSize: 10, fontWeight: 700, letterSpacing: '0.06em', color: cs.accent }}>
                          NEXT MOVE &nbsp;<span style={{ fontWeight: 400, color: '#5A6480', letterSpacing: 0 }}>{card.nextMove}</span>
                        </p>
                      </div>
                    </div>
                  )
              ...
              {discProfile && (() => {
                const discColors: Record<string, { bg: string; border: string; badge: string; text: string; dim: string }> = {
                  ...
                  S: { bg: '#F2F6FB', border: 'rgba(44,74,117,0.28)',  badge: '#2C4A75', text: '#1B3A63', dim: 'rgba(44,74,117,0.08)' },
                    ...
                    <>
                      <div style={{ display: 'flex', gap: 4, marginBottom: 9 }}>
                        {(['Negative', 'Neutral', 'Positive'] as const).map(label => {
                          ...
                          const col = label === 'Positive' ? '#2C4A75' : label === 'Negative' ? '#C0392B' : '#8C6D2F'
                          return (
                      ...
                      <div style={{ height: 4, borderRadius: 2, background: 'linear-gradient(to right, #C0392B 0%, #DDD5BB 50%, #2C4A75 100%)', position: 'relative' }}>
          ...
          <div style={{ backgroundColor: '#FDFAF5', border: '1px solid #DDD5BB', borderRadius: 16, padding: '36px 32px', maxWidth: 520, width: '100%', maxHeight: '80vh', overflowY: 'auto', boxShadow: '0 8px 40px rgba(0,0,0,0.08)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 4 }}>
              <div>
                ...
                <span style={{ fontSize: 10, fontWeight: 800, letterSpacing: '0.08em', color: '#2C4A75', backgroundColor: 'rgba(44,74,117,0.1)', padding: '2px 8px', borderRadius: 4 }}>{FOCUS_BADGE[callFocus]}</span>
              </div>
            ...
            {lines.length > 0 && (
              <div style={{ marginBottom: 16 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 5 }}>
                  <span style={{ fontSize: 11, fontWeight: 700, color: '#2C4A75' }}>YOU {agentPct}%</span>
                ...
                <div style={{ height: 6, borderRadius: 3, overflow: 'hidden', display: 'flex' }}>
                  <div style={{ width: `${agentPct}%`, backgroundColor: '#2C4A75' }} />
                  <div style={{ flex: 1, backgroundColor: '#8C6D2F' }} />
                </div>
              </div>
            )}
            ...
            {discProfile && (() => {
              const discColors: Record<string, { border: string; badge: string; text: string; dim: string }> = {
                ...
                S: { border: 'rgba(44,74,117,0.28)',  badge: '#2C4A75', text: '#1B3A63', dim: 'rgba(44,74,117,0.07)' },
            ...
            <div style={{ display: 'flex', gap: 10, marginTop: 8 }}>
              <button
                ...
                style={{ flex: 1, padding: '10px 0', borderRadius: 10, backgroundColor: '#14213D', color: '#C8D2E0', fontWeight: 700, fontSize: 13, border: 'none', cursor: 'pointer', fontFamily: 'inherit' }}
              >
                New Call
              </button>
